r"""Utility to extract DP05 demographic variables from the public ACS dataset
...
Usage examples (from the workspace root):
  # write RI tract DP05 variables to a BigQuery table
  python src\load_dp05.py --state-fips 44 --bq-dataset ml_final --bq-table dp05_ri --write-bq

  # write RI tract DP05 variables to a local CSV
  python src\load_dp05.py --state-fips 44 --local-csv outputs\tables\dp05_ri.csv

The script will attempt to auto-detect a DP05 table in the
...
def find_dp05_table(client, candidate=None):
    ...
    # fallback: look for censustract table (contains demographic data equivalent to DP05)
    print("No explicit DP05 table name provided; searching for census tract demographic tables...")
    sql = """
    SELECT table_name
    ...
    if not names:
        raise RuntimeError("No DP05-like or census tract table found in the public ACS dataset")
...
def get_dp05_columns(client, table_name):
    ...
    the most useful demographic indicators that align with DP05 concepts.
    """
...
def main():
    ...
    parser.add_argument('--table-name', default=None, help='Exact DP05 table name if known')
    ...
    parser.add_argument('--bq-table', default='dp05_ri', help='Destination table name in your project')
    ...
    dp05_table = find_dp05_table(client, args.table_name)
    public_table = f"{public_prefix}.{dp05_table}"
    print('Using public table:', public_table)

    cols = get_dp05_columns(client, dp05_table)
    if not cols:
        raise RuntimeError('No DP05 columns found')
